...
public class Day5
{
    ...
    public static long Sol1()
    {
        ...
        var ventPlot = new VentPlot(segments);
        ventPlot.PlotVents();
...
public class VentPlot
{
    ...
    public VentPlot(List<VentSegment> segments)
    {
        var x = Math.Max(segments.Max(x => x.EndPoint.X), segments.Max(x => x.StartPoint.X)) + 1;
        var y = Math.Max(segments.Max(x => x.EndPoint.Y), segments.Max(x => x.StartPoint.Y)) + 1;
        Grid = new int[y, x];
        Segments = segments;
    }
...
public class VentSegment
{
    ...
    {
        if (IsVertical && StartPoint.Y > EndPoint.Y)
            SwapPoints();
        else if (IsHorizontal && StartPoint.X > EndPoint.X)
            SwapPoints();
        else if ((StartPoint.X > EndPoint.X && StartPoint.Y > EndPoint.Y) || (StartPoint.X > EndPoint.X && StartPoint.Y < EndPoint.Y))
            SwapPoints();
    }
